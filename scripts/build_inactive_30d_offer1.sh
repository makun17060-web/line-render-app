...
with last_open as (
  select user_id, max(opened_at) as last_open_at
  from liff_open_logs
...
targets as (
  select u.user_id
  from users u
  left join last_open lo on lo.user_id = u.user_id
  where
    (lo.last_open_at is null or lo.last_open_at < now() - interval '30 days')
    ...
    and not exists (
      ...
      where o.user_id = u.user_id
        and (
          ...
          or (o.status in ('paid','confirmed','pickup'))
        )
    )
    and not exists (
...
select '${SEGMENT_KEY}', t.user_id, now()
from targets t
