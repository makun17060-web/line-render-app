...
/**
 * ✅ 送料DBキャッシュ（5分）
 * - shipping_yamato_taxed があれば全件ロードして高速化
 * - 無ければ都度 SELECT、さらに失敗したらフォールバック
 */
const shippingCache = {
  loadedAt: 0,
...
async function reloadShippingCacheIfNeeded(pool) {
  ...
  try {
    ...
    // テーブル未作成などは無視（ensureDbで作るが、念のため）
    // logErrはうるさくなるのでここでは出さない
  }
}
