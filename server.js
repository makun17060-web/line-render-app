...
// + イプシロン決済 /api/pay + 入金通知 /api/epsilon/notify
...
//       EPSILON_CONTRACT_CODE, EPSILON_ST_CODE, EPSILON_ORDER_URL, EPSILON_DEFAULT_MAIL,
//       EPSILON_SUCCESS_URL, EPSILON_FAILURE_URL
      ...
      footer: {
        ...
        contents: [
            ...
            contents: [
              {
                type: "button",
                style: "secondary",
                action: { type: "postback", label: "-1", data: `order_qty?${qstr({ id, qty: Math.max(1, q - 1) })}` }
              },
              {
                type: "button",
                style: "secondary",
                action: { type: "postback", label: "+1", data: `order_qty?${qstr({ id, qty: Math.min(99, q + 1) })}` }
              },
            ]
          },
          {
          ...
          },
          {
            type: "button",
            style: "primary",
            action: { type: "postback", label: "受取方法へ", data: `order_method?${qstr({ id, qty: q })}` }
          },
          {
            type: "button",
            style: "secondary",
            action: { type: "postback", label: "← 商品一覧", data: "order_back" }
          }
        ]
      }
    }
        ...
        contents: [
          {
            ...
            action: {
              ...
              data: `order_confirm_view?${qstr({ id, qty, method, region, payment: "cod" })}`
            }
          },
          {
            type: "button",
            ...
            action: {
              type: "postback",
              label: "銀行振込",
              data: `order_confirm_view?${qstr({ id, qty, method, region, payment: "bank" })}`
            }
          }
        ]
      }
...
async function payWithEpsilon(req, res) {
  try {
    ...
    const orderNumber = String(Date.now()).replace(/[^0-9]/g, "").slice(0, 32);
  ...
  try {
    ...
    for (let i=0; i<n && idx < total; i++, idx++) {
      ...
      try {
        ...
        console.error("notify-next push error:", e?.response?.data || e);
      }
    }
    state[pid].idx = idx;
    state[pid].updatedAt = new Date().toISOString();
  ...
  try {
    ...
    for (let i = 0; i < userIds.length; i += chunkSize) {
      ...
      try {
        ...
        sent += ids.length; batches++;
      } catch (e) {
        const detail = e?.response?.data || String(e);
        console.error("send-flex multicast error:", detail);
        results.push({ ok:false, size: ids.length, error: detail });
        failed += ids.length; batches++;
      }
    }
